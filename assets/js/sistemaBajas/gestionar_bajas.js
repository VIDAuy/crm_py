...
function MIDBactualizarDatos() {
	...
	if (nombre_funcionario == '') {
		...
		$.ajax({
			type: "POST",
			url: `${url_ajax}sistemaBajas/actualizarBaja.php`,
			data: {
				usuario: usuario,
...
function corroborarBajas(usuario_logueado) {
	...
	$.ajax(
		{
			url: `${url_ajax}sistemaBajas/listarBajas.php?usuario=${usuario_logueado}`,
			dataType: 'JSON',
			beforeSend: function () {
				$('#where').val('2');
			},
	...
	$.ajax({
		...
		success: function (content) {
			if (content.error != true) {
				...
				error('Actualmente no hay bajas que apliquen con ese filtro');
			}
		}
	});
}
